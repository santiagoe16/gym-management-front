...
export const FingerprintEnrollmentModal: React.FC<FingerprintEnrollmentModalProps> = ({ isOpen, onClose, userId, userName }) => {
  const { sendMessage, lastMessage, isConnected } = useWebSocket();
  const [status, setStatus] = useState<EnrollmentStatus>('idle');
  const [message, setMessage] = useState('Iniciando proceso de enrolamiento...');
  ...
  useEffect(() => {
    ...
    if (isOpen && userId) {
      if (!isConnected) {
        ...
        setMessage('No se pudo conectar con el servicio de huellas. Asegúrate de que esté activo y recarga la página.');
        return;
      }
      ...
      // Iniciar la comunicación con el backend para el enrolamiento
        console.log("Iniciando enrolamiento para", JSON.stringify({ type: 'user', id: userId }));
  ...
  useEffect(() => {
    ...
    switch (data.type) {
      case 'user_established':
        ...
        setMessage('Servidor listo. Por favor, coloque el primer dedo en el lector.');
        break;
      ...
      case 'capture_success':
        if (status === 'capturing_first_finger' || status === 'first_finger_enrolled') {
      ...
      case 'first_finger_enrolled':
        ...
        setMessage('Primer dedo registrado. Ahora coloque el segundo dedo en el lector.');
        break;
  ...
  useEffect(() => {
    if (status === 'capturing_first_finger') {
      setMessage(`¡Captura exitosa! (${firstFingerCaptures}/${capturesNeeded})`);
    } else if (status === 'capturing_second_finger') {
      setMessage(`¡Captura exitosa! (${secondFingerCaptures}/${capturesNeeded})`);
    }
  }, [firstFingerCaptures, secondFingerCaptures, status, capturesNeeded]);


  const handleClose = () => {
    if (status !== 'enrollment_completed' && status !== 'error' && isConnected) {
  ...
  const renderContent = () => {
    switch (status) {
      case 'ws_disconnected':
        return (
            <div className="flex flex-col items-center justify-center p-8 text-center">
                ...
                <h3 className="text-2xl font-bold mt-4 text-gray-800">Sin Conexión</h3>
      ...
      case 'capturing_second_finger':
        const captures = status === 'capturing_first_finger' ? firstFingerCaptures : secondFingerCaptures;
        const step = status === 'capturing_first_finger' ? 1 : 2;
        return (
          <div className="flex flex-col items-center justify-center p-8">
            <Fingerprint className="w-24 h-24 text-blue-500 animate-pulse" />
            <h3 className="text-xl font-semibold mt-4">Registrando {step === 1 ? 'Primer' : 'Segundo'} Dedo</h3>
            <p className="mt-2 text-gray-600">{message}</p>
            ...
            <p className="mt-2 font-bold text-2xl">{captures}/{capturesNeeded}</p>
          </div>
        );
      case 'enrollment_completed':
        return (
    ...
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md m-4 max-h-[500px]">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-800">Ingresar Huella Digital</h2>
        ...
        <div className="px-6 py-4 bg-gray-50 rounded-b-lg flex justify-end">
          ...
          >
            {status === 'enrollment_completed' || status === 'error' ? 'Cerrar' : 'Cancelar'}
          </button>
        </div>
      </div>
    </div>
